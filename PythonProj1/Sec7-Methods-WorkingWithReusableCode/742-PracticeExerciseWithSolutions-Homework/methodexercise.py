...
def calculateNetIncome(gross, state):
    ...
    :return: Net Income
    """
    state_tax = {'CA': 10, 'NY': 9, 'TX': 0, 'NJ': 6} #dictionary list

    # Calculate net income after federal tax
    net = gross - (gross * .10)# the value will be passed here after calling the method(calculatenetIncome)
    #print("The net income for the state of " + calculateNetIncome(state) +"  is " + str(net))
